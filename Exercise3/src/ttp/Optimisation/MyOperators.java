...
//			}
	...
	public static TTPSolution ModifiedLocalSearch(TTPInstance instance, int[] tour, int durationToImprovement, int maxRuntime, int[] packingPlan){
		...
		while(true) {
			...
			double Length = 1;
			if(counter > 5000) {
				Length = (double)newPackingPlan.length * 0.0001d;
			}
			else {
				Length = 1;
			}
	...
	/**
	 * This function adopt from ComplexHeuristic_2 algorithm, change the ModifiedEA part into dynamicEA and change the time given to each part
	 * @param instance
	 * @param tour
	 * @param durationWithoutImprovement
	 * @param maxRuntime
	 * @return
	 */
	public static TTPSolution ComplexHeuristic_3(TTPInstance instance, int[] tour,  int durationWithoutImprovement, int maxRuntime) {
		ttp.Utils.Utils.startTiming();
		//set up time clock
		long startTime_G = System.currentTimeMillis();
		...
		// distribute time to 2 approaches after the greedy algorithm finished.
		int lefttime = maxRuntime - (int)(System.currentTimeMillis() - startTime_G); 
		int phase2_Time = (int) (lefttime * 0.4);
		int phase3_Time = (int) (lefttime * 0.6);
			

		
		// Improve the result from dynamicEA
		TTPSolution result_dEA = dynamicEA(instance, tour, durationWithoutImprovement, phase2_Time, tempSolution.packingPlan);	
		// Improve the result  using bitflip
		TTPSolution	result_Final =  bitFlip(instance, tour, durationWithoutImprovement, phase3_Time, result_dEA.packingPlan);	
	...
	/**
	 * This function is bitflip local search algorithm adopt form given package, this algorithm will use up the set up time
	 * @param instance
	 * @param tour
	 * @param durationToImprovement
	 * @param maxRuntime
	 ...
	 * @return
	 */
	public static TTPSolution bitFlip(TTPInstance instance, int[] tour, int durationToImprovement,
			int maxRuntime, int[] packingPlan){
		// parameter setup
		int index = 0;
		int counter = 0;
		// setup time clock
		...
		while(true) {
            ...
            // flip one bit
            int position1 = (int)(Math.random()*newPackingPlan.length);
//            newPackingPlan[position] = Math.abs(newPackingPlan[position]-1);
            if (newPackingPlan[position1] == 1) {
                        newPackingPlan[position1] = 0;
                        // investigation: was at least one item flipped to zero during an improvement?
//                        flippedToZero = true;
            } else {
                newPackingPlan[position1] = 1;
            }
			// check out the new solution objective value
			TTPSolution NextSolution = new TTPSolution(tour, newPackingPlan);
			instance.evaluate(NextSolution);
	...
	/**
	 * This function is adopted from (1+1) EA, make the mutation possibility dynamic changed with the left pack capability 
	 * The (1+1)EA is EA adopted from the given Package()
	 * @param instance
	 * @param tour
	 * @param durationToImprovement
	 ...
	 * @return
	 */
	public static TTPSolution dynamicEA(TTPInstance instance, int[] tour, int durationToImprovement, int maxRuntime, int[] packingPlan){
		// parameter setup
		int index = 0;
		// setup time clock
		long LS_Start_Time = System.currentTimeMillis();
		...
		double posibility = currentSolution.wend/instance.capacityOfKnapsack;
		...
		while(true) {
            ...
            for (int j=0; j<packingPlan.length; j++) {
            	// flip with probability 1/n
                if (Math.random()<(1d/packingPlan.length + posibility))
                	// flip status based on current status
                    if (newPackingPlan[j] == 0) {
                        newPackingPlan[j] = 1;
                        changed = true;
