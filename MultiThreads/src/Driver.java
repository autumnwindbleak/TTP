...
public class Driver {
	...
	public static void main(String[] args) throws IOException {
		// create new executor services
		// setup my threads
		for(int tmp = 9; tmp >= 1; tmp--) {
			ExecutorService executor = Executors.newFixedThreadPool(MYTHREADS);

			for (int i = 0; i < 1; i++) {
				// start threads computing
				Runnable worker = new MyRunnable(String.valueOf(tmp));
	...
	public static void DynamicItems(String[] args) throws IOException {
		...
		for (File f:files) {
			...
			for (int currentTimeOfRun = 1; currentTimeOfRun <= totRuns; ++currentTimeOfRun) {
				...
				for (int currentGeneration = 1; currentGeneration <= totalGeneration; currentGeneration=currentGeneration + 50) {        	
	...
	/**
	 * Multi-threads for parallel running multiple runs
	 * @author puzhiyao
	 *
	 */
	public static class MyRunnable implements Runnable {
		private final String thread_InstanceIndex;
		/**
		 * Setup new instance index
		 * @param thread_InstanceIndex
		 */
		MyRunnable(String thread_InstanceIndex) {
			this.thread_InstanceIndex = thread_InstanceIndex;
		}

		@Override
		public void run() {
			// tmp string arguments
			String args[];
			// run configuration
			...
			parameters[1] = thread_InstanceIndex;
